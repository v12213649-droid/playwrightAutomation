...
export class AddEmployeePage {
  ...
  constructor(page: Page) {
    ...
    this.nextButtonStep1 = page.locator('button.add-btn:has-text("Next"), button:has-text("Next")').first();
    ...
    this.nextButtonStep2 = page.locator('button.add-btn:has-text("Next"), button:has-text("Next")').first();
    ...
    this.nextButtonStep3 = page.locator('button.add-btn:has-text("Next"), button:has-text("Next")').first();

    // Step 4: Documents & Final Submit
    this.profileImageFileInput = page.locator('input.file-upload-input, input[type="file"]').first();
    this.submitAddEmployeeButton = page.locator('button.add-btn:has-text("Add Employee"), button:has-text("Add Employee")').last();
  }
  ...
  async selectDropdownOption(dropdown: Locator, optionText: string, timeout = 30000) {
    ...
    while (Date.now() < deadline) {
      try {
        ...
        const partialOption = listbox.locator('li[role="option"], p-selectitem').filter({ hasText: optionText }).first();
  ...
  async selectFilterableDropdownOption(dropdown: Locator, optionText: string, timeout = 30000) {
    ...
    while (Date.now() < deadline) {
      try {
        ...
        const option = listbox.locator('li[role="option"], p-selectitem').filter({ hasText: new RegExp(optionText, 'i') }).first();
        if (await option.isVisible({ timeout: 2000 }).catch(() => false)) {
          await option.click();
          return;
        }

        const firstOption = listbox.locator('li[role="option"]:visible').first();
        if (await firstOption.isVisible({ timeout: 1000 }).catch(() => false)) {
          await firstOption.click();
          return;
        }
  ...
  async uploadProfileImageAndSubmit(filePath?: string) {
    // 45. Profile Image
    if (filePath) {
      await this.profileImageFileInput.waitFor({ state: 'attached', timeout: 15000 });
      await this.profileImageFileInput.setInputFiles(filePath);
      await this.page.waitForTimeout(500);
    }

    // 46. Add Employee Submit Button
  ...
  async createEmployee(details: EmployeeDetails) {
    ...
    await this.uploadProfileImageAndSubmit(details.profileImagePath);
  }
}
