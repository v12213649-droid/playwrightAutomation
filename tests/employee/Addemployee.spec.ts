...
test.describe('Employee Module - Add Employee', () => {
  test('should add a new employee following the 46-step wizard sequence with unique data', async ({ page }) => {
    ...
    const aadhaarFrontPath = DataGenerator.documentImage(`${firstName}-aadhaar-front`);
    const aadhaarBackPath = DataGenerator.documentImage(`${firstName}-aadhaar-back`);
    ...
    // ==========================================
    console.log('Step 4: Uploading profile image and Aadhaar documents, then submitting...');
    await employeePage.uploadProfileImageAndSubmit(profileImagePath, aadhaarFrontPath, aadhaarBackPath);
